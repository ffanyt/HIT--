class Data:
    ...
    def check(full_data, len_of_single_cal=8):
        ...
        # 计算校验和
        checksum = 0
        bin_of_package = bin(int.from_bytes(data, byteorder="big")) # 将data转换为二进制
        len_of_package = len(bin_of_package) - 2 # 减去0b
        if len_of_package % len_of_single_cal != 0: # 如果数据长度不是len_of_single_cal的整数倍
            bin_of_package += "0" * (len_of_single_cal - len_of_package % len_of_single_cal) # 补0
        for i in range(2, len_of_package, len_of_single_cal): # 从第二位开始，每次取len_of_single_cal位
            block = int(bin_of_package[i:i + len_of_single_cal], 2) # 将取到的二进制转换为十进制
            checksum += block # 相加
            if checksum > 0xff: # 如果相加结果超过len_of_single_cal位
                checksum = (checksum & 0xff) + 1 # 将溢出的高位加到低位
