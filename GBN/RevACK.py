    ...
    def run(self):
        ...
        while True:
            ...
            if ack_id is not None:
                end_flag = True
            if self.ackMap.get(ack_id) is not None: # 如果ack_id在ackMap中
                if self.ackMap[ack_id] is False: # 如果ack_id对应的值为False
                    ack_map_before = self.ackMap.copy() # 复制ackMap
                    for i in range(ack_id): # 将ack_id之前的所有值设为True
